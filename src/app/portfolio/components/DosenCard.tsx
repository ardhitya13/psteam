  ...
  const dosenList = [
    {
      ...
      educationLevel: "Magister Strata 2 (S2)",
    ...
    },
    {
      ...
      educationLevel: "Doktor Strata 3 (S3)",
...
function DosenSingleCard({ dosen }: any) {
  ...
  const filteredData = useMemo(() => {
    ...
    return data;
  }, [dataAktif, activeTab, sortOrder, typeFilter]);

  const totalPages = Math.ceil(filteredData.length / itemsPerPage);
  const currentData = filteredData.slice((page - 1) * itemsPerPage, page * itemsPerPage);

  return (
    <article className="bg-white rounded-3xl shadow-lg border border-blue-200 overflow-visible transition-all duration-300 hover:shadow-2xl">
      {/* HEADER */}
        ...
        <ul className="list-disc ml-5 space-y-1">
          {dosen.educationHistory.map((edu: any, i: number) => (
            <li key={i}>
              ...
              <span className="italic">{edu.major}</span>
            </li>
          ))}
        </ul>
      </div>
      ...
      {/* TABEL */}
      <div className="overflow-x-auto px-6 pb-4 relative z-0">
        <div className="border border-blue-100 rounded-lg shadow-sm bg-white relative overflow-visible">
          <table className="w-full text-sm text-black">
                  ...
                  <th className="p-3 text-left font-semibold relative">
                    ...
                    <div ref={dropdownRef} className="inline-block ml-2 relative">
                      <button
                        ...
                        className="text-xs text-blue-700 border border-blue-400 bg-blue-50 hover:bg-blue-100 px-2 py-1 rounded-md transition-all"
                      >
            ...
            <tbody>
              {currentData.map((item: any, i: number) => (
                <tr key={i} className="even:bg-blue-50/30 hover:bg-blue-100/50 transition-colors">
      ...
      {/* PAGINATION */}
      {totalPages > 1 && (
        <div className="flex justify-center gap-2 py-6 rounded-b-3xl">
          <button
            onClick={() => setPage(Math.max(page - 1, 1))}
            disabled={page === 1}
            className={`px-3 py-1.5 rounded-md border text-sm font-semibold transition-all duration-300 ${page === 1
                ? "text-gray-400 border-gray-200 cursor-not-allowed"
                : "text-blue-700 border-gray-300 hover:bg-blue-50 hover:border-blue-400"
              }`}
          >
            &lt;
          </button>
          ...
          {Array.from({ length: totalPages }, (_, i) => (
            <button
              key={i}
              onClick={() => setPage(i + 1)}
              className={`px-3 py-1.5 rounded-md border text-sm font-semibold transition-all duration-300 ${page === i + 1
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-md border text-sm font-semibold transition-all duration-300 ${page === totalPages
                ? "text-gray-400 border-gray-200 cursor-not-allowed"
                : "text-blue-700 border-gray-300 hover:bg-blue-50 hover:border-blue-400"
              }`}
          >
