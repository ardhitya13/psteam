...
export default function DaftarKaryaIlmiahPage() {
    ...
    const [data, setData] = useState<KaryaIlmiahItem[]>(() => [
        { no: 1, judul: "Interpretable Machine Learning for Job Placement Prediction", jenis: "Jurnal Nasional Terakreditasi", tahun: 2025, status: "Sudah Diterbitkan" },
        { no: 2, judul: "Penerapan Teknologi Raspberry Pi dalam Monitoring Kehadiran", jenis: "Jurnal Nasional", tahun: 2025, status: "Belum Diterbitkan" },
        { no: 3, judul: "Classification of Alzheimer Disease from MRI Image", jenis: "Prosiding Seminar Internasional", tahun: 2023, status: "Sudah Diterbitkan" },
        ...Array.from({ length: 10 }, (_, i): KaryaIlmiahItem => ({
            no: i + 4,
            judul: `Judul Karya Ilmiah ${i + 4}`,
            jenis: i % 2 === 0 ? "Jurnal Nasional Terakreditasi" : "Prosiding Nasional",
            tahun: 2020 + ((i + 4) % 6),
    ...
    const handleTerbitkan = (no: number) => {
        setData((prev) =>
            prev.map((item) => (item.no === no ? { ...item, status: "Sudah Diterbitkan" } : item))
        );
    };
                    ...
                    <table className="w-full border-collapse text-sm text-gray-700">
                        <thead className="bg-gray-300 text-gray-800">
                            <tr>
                                ...
                                <th className="border border-gray-200 px-4 py-2 text-center">JENIS KARYA</th>
