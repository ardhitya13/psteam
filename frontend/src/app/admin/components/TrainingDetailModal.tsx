        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-5">
          <div>
            ...
            <div className="mt-4">
              ...
              <p className="text-lg font-semibold text-orange-600">Rp {Number(data.price || 0).toLocaleString()}</p>
            </div>

            <div className="mt-3 text-xs text-gray-500">Penyelenggara: {data.organizer || "-"}</div>

            {data.specification && <div className="mt-3 text-sm text-gray-700">Spesifikasi: {data.specification}</div>}
            {data.duration && <div className="mt-1 text-sm text-gray-700">Durasi: {data.duration}</div>}
            {data.location && <div className="mt-1 text-sm text-gray-700">Lokasi: {data.location}</div>}
            {data.instructor && <div className="mt-1 text-sm text-gray-700">Instruktur: {data.instructor}</div>}
            {data.certificate && <div className="mt-1 text-sm text-gray-700">Sertifikat: {data.certificate}</div>}
          </div>
          ...
          <div className="md:col-span-2 space-y-4">
              ...
              <table className="min-w-full text-sm mt-2 border-collapse">
                ...
                <tbody>
                  {data.schedule?.length ? data.schedule.map((s, i) => (
                    ...
                    <tr><td colSpan={3} className="py-3 text-center italic text-gray-500">Belum ada jadwal</td></tr>
                  )}
                </tbody>
              </table>
            </div>
            ...
            <div>
              ...
              <table className="min-w-full text-sm mt-2">
                ...
                <tbody>
                  {data.rundown?.length ? data.rundown.map((r, i) => (
                    <tr key={i}>
                      <td className="py-2 px-3 border">{r.day}</td>
                      <td className="py-2 px-3 border">{r.activity}</td>
                    </tr>
                  )) : (
                    <tr><td colSpan={2} className="py-3 text-center italic text-gray-500">Belum ada rundown</td></tr>
                  )}
                </tbody>
              </table>
            </div>
