...
export default function EditTrainingModal({ data, onClose, onUpdate }: Props) {
  ...
  // 🔵 TAMBAHAN: Modal konfirmasi simpan
  ...
  const addSchedule = () =>
    setForm(prev => ({
      ...
      schedule: [
        ...(prev.schedule || []),
        { batchName: `Batch ${(prev.schedule || []).length + 1}`, startDate: "", endDate: "" }
      ]
    }));

  const updateSchedule = (idx: number, key: any, val: string) =>
    setForm(prev => ({
      ...prev,
      schedule: prev.schedule?.map((s, i) => (i === idx ? { ...s, [key]: val } : s)) || []
    }));
  ...
  const addRundown = () =>
    setForm(prev => ({
      ...
      rundown: [
        ...(prev.rundown || []),
        { day: `Hari ${(prev.rundown || []).length + 1}`, activity: "" }
      ]
    }));

  const updateRundown = (idx: number, key: any, val: string) =>
    setForm(prev => ({
      ...prev,
      rundown: prev.rundown?.map((r, i) => (i === idx ? { ...r, [key]: val } : r)) || []
    }));

  const removeRundown = (idx: number) =>
    setForm(prev => ({
      ...prev,
      rundown: prev.rundown?.filter((_, i) => i !== idx) || []
    }));

  // 🔵 DIUBAH: submit → buka modal konfirmasi
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center p-6">
      ...
      <div className="relative w-full max-w-4xl bg-white rounded-lg shadow-lg overflow-auto max-h-[90vh] p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Semua bagian di bawah ini tetap 100% sama */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <div className="flex justify-between items-center">
                <label className="text-sm font-medium">Rincian Biaya</label>
                <button type="button" onClick={() => update("costDetails", [...(form.costDetails || []), ""])} className="text-sm text-blue-600">
                  + Tambah
                </button>
              </div>
                  ...
                  <div key={i} className="flex gap-2">
                    <input
                      ...
                      onChange={(e) =>
                        update(
                          "costDetails",
                          form.costDetails?.map((v, idx) => (idx === i ? e.target.value : v)) || []
                        )
                      }
                    ...
                    <button
                      type="button"
                      onClick={() => update("costDetails", (form.costDetails || []).filter((_, idx) => idx !== i))}
            ...
            <div>
              <div className="flex justify-between items-center">
                ...
                <button type="button" onClick={() => update("requirements", [...(form.requirements || []), ""])} className="text-sm text-blue-600">
                  + Tambah
                </button>
              </div>
              <div className="mt-2 space-y-2">
                  ...
                  <div key={i} className="flex gap-2">
                    <input
                      ...
                      onChange={(e) =>
                        update(
                          "requirements",
                          form.requirements?.map((v, idx) => (idx === i ? e.target.value : v)) || []
                        )
                      }
                      className="flex-1 border rounded px-2 py-1 text-sm"
                    />
                    <button
                      type="button"
                      onClick={() => update("requirements", (form.requirements || []).filter((_, idx) => idx !== i))}
                      className="bg-red-500 text-white px-2 rounded text-xs"
                    >
                      Hapus
                    </button>
          ...
          <div>
            <div className="flex justify-between items-center">
              ...
              <button type="button" onClick={addSchedule} className="text-sm text-blue-600">
                + Tambah Batch
              </button>
            </div>
            <div className="mt-2 space-y-2">
              {(form.schedule || []).map((s, i) => (
                <div key={i} className="flex gap-2 items-end">
                  ...
                  <input
                    ...
                    className="border rounded px-2 py-1 text-sm"
                  />
                  <button type="button" onClick={() => removeSchedule(i)} className="bg-red-500 text-white px-2 py-1 rounded text-xs">
                    Hapus
                  </button>
                </div>
              ))}
          ...
          <div>
            <div className="flex justify-between items-center">
              ...
              <button type="button" onClick={addRundown} className="text-sm text-blue-600">
                + Tambah Hari
              </button>
            </div>
            <div className="mt-2 space-y-2">
              {(form.rundown || []).map((r, i) => (
                <div key={i} className="flex gap-2 items-center">
                  ...
                  <input
                    ...
                    className="flex-1 border rounded px-2 py-1 text-sm"
                  />
                  <button type="button" onClick={() => removeRundown(i)} className="bg-red-500 text-white px-2 py-1 rounded text-xs">
                    Hapus
                  </button>
                </div>
              ))}
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[999] p-4">
          <div className="bg-white rounded-lg shadow-lg w-full max-w-sm p-6">
            ...
            <p className="text-sm text-gray-700 mb-6">
              Simpan perubahan pelatihan <b>{form.title}</b>?
            </p>
