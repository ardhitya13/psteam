...
export default function DaftarPengabdianPage() {
  ...
  const [data, setData] = useState<CommunityServiceItem[]>([]);
  const [selectedItem, setSelectedItem] =
    useState<CommunityServiceItem | null>(null);
  ...
  const fetchData = async () => {
    try {
      const res = await getMyCommunityService();
  ...
  const handleEditData = async (data: {
    ...
    const confirmEdit = await Swal.fire({
      ...
      html: `
        <b>Judul:</b> ${selectedItem?.title} → <b>${data.title}</b><br/>
        <b>Tahun:</b> ${selectedItem?.year} → <b>${data.year}</b>
      `,
    ...
    await updateCommunityService(data.id, {
      title: data.title,
      year: data.year,
    });
  ...
  const handleDeleteData = async (id: number) => {
    ...
    const confirmDelete = await Swal.fire({
      icon: "warning",
      title: "Yakin ingin menghapus data pengabdian ini?",
      html: `
