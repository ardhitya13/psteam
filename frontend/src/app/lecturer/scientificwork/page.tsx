...
export default function DaftarKaryaIlmiahPage() {
  ...
  const [selectedItem, setSelectedItem] =
    useState<ScientificWorkItem | null>(null);
  ...
  const fetchData = async () => {
    try {
      const res = await getMyScientificWork();
  ...
  const handleAdd = async (payload: {
    ...
    try {
      await addScientificWork(payload);
      setIsAddOpen(false);
      await fetchData();
  ...
  /* ================= EDIT ================= */
  const handleEdit = async (updated: ScientificWorkItem) => {
    ...
    await updateScientificWork(updated.id, {
      title: updated.title,
      type: updated.type,
      year: updated.year,
    });
    ...
    Swal.fire({
      icon: "success",
      title: "Berhasil",
      text: "Data karya ilmiah berhasil diperbarui",
    });
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    await deleteScientificWork(id);
    await fetchData();

    Swal.fire({
      icon: "success",
      title: "Dihapus",
      text: "Data karya ilmiah berhasil dihapus",
    });
  };
        ...
        >
          <div className="text-center mb-8">
            <h1 className="text-3xl font-bold text-black uppercase">
              Daftar Kaya Ilmiah Dosen
            </h1>
            <p className="text-gray-600 text-sm">
              Detail karya dosen — edit dan melalui modal.
            </p>
          </div>
